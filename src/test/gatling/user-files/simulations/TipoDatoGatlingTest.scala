...
/**
 * Performance test for the TipoDato entity.
 */
class TipoDatoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TipoDato entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all tipoDatoes")
            .get("/services/pruebams/api/tipo-datoes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new tipoDato")
            .post("/services/pruebams/api/tipo-datoes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nombre":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_tipoDato_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created tipoDato")
                .get("/services/pruebams${new_tipoDato_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created tipoDato")
            .delete("/services/pruebams${new_tipoDato_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
