...
/**
 * Performance test for the Dato entity.
 */
class DatoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Dato entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all datoes")
            .get("/services/pruebams/api/datoes")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new dato")
            .post("/services/pruebams/api/datoes")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nombre":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_dato_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created dato")
                .get("/services/pruebams${new_dato_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created dato")
            .delete("/services/pruebams${new_dato_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
