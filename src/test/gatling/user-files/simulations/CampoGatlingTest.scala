...
/**
 * Performance test for the Campo entity.
 */
class CampoGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Campo entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all campos")
            .get("/services/pruebams/api/campos")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new campo")
            .post("/services/pruebams/api/campos")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nombre":"SAMPLE_TEXT"
                ...
                , "requerido":null
                , "longitud":"0"
                , "dependienteVisibilidad":null
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_campo_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created campo")
                .get("/services/pruebams${new_campo_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created campo")
            .delete("/services/pruebams${new_campo_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
