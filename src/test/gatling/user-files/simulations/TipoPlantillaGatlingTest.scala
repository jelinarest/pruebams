...
/**
 * Performance test for the TipoPlantilla entity.
 */
class TipoPlantillaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the TipoPlantilla entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all tipoPlantillas")
            .get("/services/pruebams/api/tipo-plantillas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new tipoPlantilla")
            .post("/services/pruebams/api/tipo-plantillas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nombre":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_tipoPlantilla_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created tipoPlantilla")
                .get("/services/pruebams${new_tipoPlantilla_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created tipoPlantilla")
            .delete("/services/pruebams${new_tipoPlantilla_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
