...
/**
 * Performance test for the Cronograma entity.
 */
class CronogramaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Cronograma entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cronogramas")
            .get("/services/pruebams/api/cronogramas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cronograma")
            .post("/services/pruebams/api/cronogramas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nombre":"SAMPLE_TEXT"
                , "descripcion":"SAMPLE_TEXT"
                , "motivoAjuste":"SAMPLE_TEXT"
                , "activo":null
                , "fechaAlta":"2020-01-01T00:00:00.000Z"
                }""")).asJson
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cronograma_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cronograma")
                .get("/services/pruebams${new_cronograma_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cronograma")
            .delete("/services/pruebams${new_cronograma_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
