...
/**
 * Performance test for the CronogramaEtapa entity.
 */
class CronogramaEtapaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the CronogramaEtapa entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all cronogramaEtapas")
            .get("/services/pruebams/api/cronograma-etapas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new cronogramaEtapa")
            .post("/services/pruebams/api/cronograma-etapas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "nombre":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_cronogramaEtapa_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created cronogramaEtapa")
                .get("/services/pruebams${new_cronogramaEtapa_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created cronogramaEtapa")
            .delete("/services/pruebams${new_cronogramaEtapa_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
